...
{
	CMatrix4d transform;
	transform.Translate(2, 1.3, 0);
	transform.Rotate(-110, 0, 1, 0);
	CSimpleMaterial blue;
	blue.SetDiffuseColor(CVector4f(0.5f, 0.8f, 1, 1));

	AddIcosahedron(CreateSimpleDiffuseShader(blue), transform);
}
...
CSceneObject& CRaytraceView::AddIcosahedron(
	IShader const& shader,
	CMatrix4d const& transform
)
{
	auto& icosahedron = *m_geometryObjects.emplace_back(
		std::make_unique<CIcosahedron>(transform));

	auto trans = CMatrix4d();
	trans.Translate(-5, 2, 0);
	icosahedron.SetTransform(trans);
