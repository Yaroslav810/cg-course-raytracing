...
void CRaytraceView::AddSomeTetrahedron()
{
	...
	CMatrix4d transform;
	transform.Translate(0, 0.3, 2);
	transform.Rotate(170, 0, 1, 0);
	CSimpleMaterial blue;
	blue.SetDiffuseColor(CVector4f(0.5f, 0.8f, 1, 1));

	AddTetrahedron(CreateSimpleDiffuseShader(blue), v1, v2, v3, v4, transform);
}
...
{
	CMatrix4d transform;
	transform.Translate(-2, 2.3, -1);
	transform.Rotate(170, 0, 1, 0);
	CSimpleMaterial blue;
	blue.SetDiffuseColor(CVector4f(0.5f, 0.8f, 1, 1));

	AddOctahedron(CreateSimpleDiffuseShader(blue), transform);
}
...
void CRaytraceView::AddSomeCubes()
{
	...
	yellow.SetDiffuseColor(CVector4f(1, 1, 0, 1));
	CSimpleDiffuseShader& shader = CreateSimpleDiffuseShader(yellow);
	AddCube(shader, 1, CVector3d(0, 1, 0));
	AddCube(shader, 0.5, CVector3d(2, 1, 1.5));
}
...
CSceneObject& CRaytraceView::AddCube(IShader const& shader, double size, CVector3d const& center, CMatrix4d const& transform)
{
	const auto& cube = *m_geometryObjects.emplace_back(
		std::make_unique<CCube>(size, center, transform));

	return AddSceneObject(cube, shader);
}

CSceneObject& CRaytraceView::AddTetrahedron(
...
{
	const auto& cube = *m_geometryObjects.emplace_back(
		std::make_unique<CTetrahedron>(v1, v2, v3, v4, transform));

	return AddSceneObject(cube, shader);
}

CSceneObject& CRaytraceView::AddOctahedron(
	IShader const& shader,
	CMatrix4d const& transform
)
{
	const auto& cube = *m_geometryObjects.emplace_back(
		std::make_unique<COctahedron>(transform));
