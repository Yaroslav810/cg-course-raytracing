...
// Добавляем гиперболический параболоид
void CRaytraceView::AddSomeHyperbolicParaboloid()
{
	...
	AddHyperbolicParaboloid(shader, transform);
	// AddHyperbolicParaboloid(shader);
}
...
CSceneObject& CRaytraceView::AddHyperbolicParaboloid(IShader const& shader, CMatrix4d const& transform)
{
	const auto& hyperbolicParaboloid = *m_geometryObjects.emplace_back(
		std::make_unique<CHyperbolicParaboloid>(transform));
